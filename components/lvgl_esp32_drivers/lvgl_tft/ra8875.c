...
#define HDWR_VAL (480/8 - 1)
#define VDHR_VAL (480 - 1)
...
//		{0x8E,   0x80},
 ...
 //   vTaskDelay(DIV_ROUND_UP(10, portTICK_RATE_MS));
    vTaskDelay(100);
...
//    disp_spi_change_device_speed(8000000);
    ra8875_configure_clocks(true);
  //  disp_spi_change_device_speed(-1);
    disp_spi_change_device_speed(20000000);



   // Perform a memory clear (wait maximum of 100 ticks)
 //   ra8875_write_cmd(RA8875_REG_MCLR, 0x80);
   //ra8875_write_cmd(0x00, 0x00);
 //   ra8875_write_cmd(0x01, 0x01);


 //   ra8875_write_cmd(0x80, 0);

 //   ra8875_write_data2(0x00, 0x00);
 //   ra8875_write_data(0x00, 0x00);


    /*
    for(i = 5; i != 0; i--) {
//        if ((ra8875_read_cmd(RA8875_REG_MCLR) & 0x80) == 0x00) {
        if (ra8875_read_cmd(0) == 0x75) {
//        if (ra8875_write_data2(0x00, 0x00) == 0x75) {
...
/*
    ra8875_write_cmd2(0x80, 0x88);
    ra8875_write_data(0x00, 0x00);

    ra8875_write_cmd2(0x80, 0x89);
    ra8875_write_data(0x00, 0x03);

//    ra8875_write_cmd2(0x80, 0x10);
//    ra8875_write_data(0x00, 0x0C);

    ra8875_write_cmd2(0x80, 0x04);
    ra8875_write_data(0x00, 0x81);


    ra8875_write_cmd2(0x80, init_cmds[0].cmd);
    ra8875_write_data(0x00, init_cmds[0].data);
*/

 //   ESP_LOGW(TAG, "INIT_CMDS_SIZE %d", INIT_CMDS_SIZE);


    // Send all the commands
    for (i = 0; i < INIT_CMDS_SIZE; i++) {

  //  	ra8875_write_data2(0x00, init_cmds[i].cmd);
  //  	ra8875_write_data(0x00, init_cmds[i].data);

        ra8875_write_cmd(init_cmds[i].cmd, init_cmds[i].data);
//    	ra8875_write_cmd2(0x80, init_cmds[i].cmd);
//    	ra8875_write_data(0x00, init_cmds[i].data);
...
//   rectHelper(0, 0, 800, 480, 0x001F, 1);
...
uint8_t ra8875_read_cmd(uint8_t cmd)
{
    uint8_t buf[2] = {RA8875_MODE_DATA_READ, cmd};
    disp_spi_transaction(buf, sizeof(buf), (disp_spi_send_flag_t)(DISP_SPI_RECEIVE | DISP_SPI_SEND_POLLING), (disp_spi_read_data*)buf, 0);
    return buf[1];
}
...
uint8_t ra8875_write_data2(uint8_t cmd, uint8_t data)
{
    uint8_t buf[2] = {RA8875_MODE_CMD_WRITE, cmd, RA8875_MODE_DATA_READ, data};
    disp_spi_send_data(buf, sizeof(buf));
    return buf[1];
}
...
void ra8875_write_cmd2(uint8_t cmd, uint8_t data)
{
    uint8_t buf[2] = {RA8875_MODE_CMD_WRITE, data};
    disp_spi_send_data(buf, sizeof(buf));
}

void ra8875_write_data(uint8_t cmd, uint8_t data)
{
    uint8_t buf[2] = {RA8875_MODE_DATA_WRITE, data};
    disp_spi_send_data(buf, sizeof(buf));
}
...
//    ra8875_write_cmd(RA8875_REG_PLLC1, val);           // PLL Control Register 1 (PLLC1)
    ra8875_write_cmd(RA8875_REG_PLLC1, 0x07);
    vTaskDelay(1);

//    val = high_speed ? CONFIG_LVGL_DISP_RA8875_PLLDIVK : 0x03;
//    ra8875_write_cmd(RA8875_REG_PLLC2, val);           // PLL Control Register 2 (PLLC2)
    ra8875_write_cmd(RA8875_REG_PLLC2, 0x01);
    vTaskDelay(1);

    ra8875_write_cmd(RA8875_REG_PCSR, 0x81);            // Pixel Clock Setting Register (PCSR)
    vTaskDelay(DIV_ROUND_UP(20, portTICK_RATE_MS));
}
...
*/
 ...
 // writeData(y);
  ...
  /* Draw! */
//	 ra8875_write_cmd(RA8875_DCR, 0xB0);
  if (filled) {
    //writeData(0xB0);
	  ra8875_write_cmd(RA8875_DCR, 0xB0);
  } else {
